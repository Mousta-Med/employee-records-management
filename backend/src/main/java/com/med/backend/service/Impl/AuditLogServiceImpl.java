...
public class AuditLogServiceImpl implements AuditLogService {
    ...
    public List<AuditLogRes> getLogsByEmployeeId(UUID employeeId) {
        return auditLogRepository.findAll().stream().filter(log -> log.getEmployeeId().equals(employeeId)).map(AuditLog -> modelMapper.map(AuditLog, AuditLogRes.class)).collect(Collectors.toList());
    }
